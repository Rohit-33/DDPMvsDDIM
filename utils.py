...
def viz_clf(ax, model, DEVICE=torch.device('cpu'), hard=False, 
    ...
    # For each sample, pass in the timestep `t` and class label `y`
    timesteps = torch.full((leng, width), 0, device=DEVICE, dtype=torch.long)  # Adjust based on your needs
    class_labels = torch.full((leng, width), 1, device=DEVICE, dtype=torch.long)  # Modify this for your labels
    ...
    if sk_clf:
        ...
        model.eval()
        if hard:
            preds = 1.0 * (model(model_inputs, timesteps, class_labels) > 0.5)  # Pass `timesteps` and `class_labels`
        else:
            preds = model(model_inputs, timesteps, class_labels)  # Pass `timesteps` and `class_labels`
